...
class BollingerBands(AbstractStrategy):
    def __init__(self, ticker_list, period, multiple, 
    ...
    def calculate_signal(self, bar_event):        
        ...
        if ticker_data:              
            ...
            if ticker_info is None:
                ...
                #go short   
                elif bar_event.close_price > bb.upper:
                    cur_time = get_cur_time(bar_event, self.session_type)
                    signal_event = SignalEvent(cur_time, bar_event.ticker,
            ...
            #the ticker symbol has been long/short                                       
                ...
                #close long position 
                if ticker_info.POS > 0 and bar_event.close_price > bb.upper:
                    cur_time = get_cur_time(bar_event, self.session_type)
                    signal_event = SignalEvent(cur_time, bar_event.ticker,
                                               OrderType.MARKET, Direction.SHORT, 
                                               bar_event.close_price)                                       
                ...
                elif ticker_info.POS < 0 and bar_event.close_price < bb.lower:                              
                    cur_time = get_cur_time(bar_event, self.session_type)
                    signal_event = SignalEvent(cur_time, bar_event.ticker,
                                            OrderType.MARKET, Direction.LONG, 
                                            bar_event.close_price)                
    ...
    def _subscribe_ticker(self, ticker_list):
        ...
        for ticker in ticker_list:
            tickers_data[ticker] = BB(self.period, self.multiple) 
        return tickers_data                                                                                        
